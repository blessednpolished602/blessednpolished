...
/**
 * Returns open hours for a date string.
 *   null      → explicitly closed (byDay entry is null)
 *   undefined → hours not configured or no default set
...
function hoursForDay(hours, dateStr) {
    ...
    return hours.default ?? undefined;               // default or unconfigured
}
...
export default function AdminCalendarView() {
    ...
    useEffect(() => {
        ...
        const last  = fmtDate(viewYear, viewMonth, daysInMonth);
    ...
    // ── Sync localAvail from monthAvail when date or data changes ─────────────
    useEffect(() => {
    ...
    function getDayStatus(ds) {
        ...
        for (const t of techs) {
            ...
            for (const s of slots) {
                if (!dayLocks.has(`${t.id}_${s}`)) { hasOpen = true; break; }
            }
            if (hasOpen) break;
        }
    ...
    function toggleSlot(techId, slotMin) {
        const dayLocks = monthLocks[selDate] ?? new Set();
        if (dayLocks.has(`${techId}_${slotMin}`)) return; // booked — cannot toggle
        setSaved(false);
        setLocalAvail(prev => {
    ...
    const rawDayHours = selDate && settings?.hours
        ? hoursForDay(settings.hours, selDate)
        : undefined;
            ...
            <div className="flex gap-6 items-start flex-wrap lg:flex-nowrap">
                ...
                <div className="flex-1 min-w-[300px]">
                    ...
                    <div className="flex items-center justify-between mb-3">
                        <button
                            className="btn btn-ghost px-3 py-1 text-xl leading-none"
                            onClick={prevMonth}
                    ...
                    {monthLoading ? (
                        ...
                        <div className="grid grid-cols-7 gap-px bg-neutral-100 rounded-xl overflow-hidden border border-neutral-100">
                            {/* Day-of-week headers */}
                            {DOW_SHORT.map(d => (
                                <div key={d} className="bg-white text-center text-xs font-medium text-neutral-400 py-2">
                                    {d}
                                </div>
                            ))}
                            ...
                            {Array.from({ length: firstDow }, (_, i) => (
                                <div key={`pad-${i}`} className="bg-white h-14" />
                            ))}
                            ...
                            {Array.from({ length: daysInMonth }, (_, i) => {
                                ...
                                return (
                                    <button
                                        ...
                                        disabled={isPast}
                                        className={[
                                            "bg-white h-14 flex flex-col items-center justify-center gap-1",
                                            "text-sm transition focus:outline-none",
                                    ...
                                    >
                                        ...
                                        <span className={`w-2 h-2 rounded-full ${DOT_COLOR[status] ?? "bg-neutral-200"}`} />
                                    </button>
                                );
                            })}
                        </div>
                ...
                {/* ── Drawer ── */}
                {selDate && (
                    <div className="w-full lg:w-80 xl:w-96 flex-shrink-0 border rounded-xl p-4 space-y-3 max-h-[75vh] overflow-y-auto">
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <button
                                ...
                                className="w-6 h-6 grid place-items-center rounded-full hover:bg-neutral-100 text-neutral-400 text-sm"
                                aria-label="Close"
                            >✕</button>
                        </div>
                                ...
                                {!dayLoading && drawerTechs.map(tech => {
                                    ...
                                    return (
                                        <div key={tech.id} className="border-t pt-3 space-y-2">
                                            ...
                                            {/* Confirmed bookings for this tech */}
                                            {confirmedBooks.length > 0 && (
                                                <div className="space-y-1">
                                                    {confirmedBooks.map(b => (
                                                        <div
                                                            key={b.id}
                                                            className="flex items-start justify-between rounded-lg bg-neutral-50 px-2 py-1.5 text-xs gap-2"
                                                        >
                                                            <div className="min-w-0">
                                                                <span className="font-medium">{minToTimeString(b.startMin)}</span>
                                                                <span className="text-neutral-600"> · {b.name}</span>
                                                                {b.serviceName && (
                                                                    <span className="text-neutral-400 truncate"> · {b.serviceName}</span>
                                                                )}
                                                            </div>
                                                            <button
